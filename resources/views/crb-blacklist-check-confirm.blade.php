        ...
        <div class="confirm-layout">
            ...
            <div class="order-card">
                ...
                <div class="order-card-body">
                    ...
                    <p style="font-size:.78rem;color:var(--text-light);line-height:1.6;margin-bottom:18px;"><i class="fa-solid fa-shield-halved" style="color:var(--primary-green);"></i> CRB data powered by Metropol CRB Kenya.</p>
...
function initiateStk() {
    const phoneInput=document.getElementById('mpesa_phone'),raw=phoneInput.value.replace(/\s/g,'');
...
function pollPayment(rid){let attempts=0;pollInterval=setInterval(()=>{attempts++;fetch('{{ route('check-payment-status') }}?rid='+rid).then(r=>r.json()).then(data=>{if(data.status==='completed'||data.status==='paid'){clearInterval(pollInterval);showSuccess(rid);}else if(data.status==='payment_failed'){clearInterval(pollInterval);stkError(data.payment_error||'Payment not completed.');}else if(attempts>=36){clearInterval(pollInterval);stkError('Payment timed out.');}}).catch(()=>{});},3000);}
