...
$first_name  = $ident_data['first_name'] ?? $identity['first_name'] ?? '';
$last_name   = $ident_data['surname']    ?? $ident_data['last_name'] ?? $identity['last_name'] ?? $identity['surname'] ?? '';
$other_name  = $ident_data['other_name'] ?? $identity['other_name'] ?? '';
$full_name   = trim(implode(' ', array_filter([$first_name, $other_name, $last_name]))) ?: ($req->full_name ?? 'Applicant');
$id_number   = $req->national_id;
$dob         = $ident_data['dob'] ?? $ident_data['date_of_birth'] ?? $identity['dob'] ?? null;
$gender      = $ident_data['gender'] ?? $identity['gender'] ?? null;
$citizenship = $ident_data['citizenship'] ?? $identity['citizenship'] ?? 'Kenyan';
$id_serial   = $ident_data['serial_number'] ?? $ident_data['identity_number'] ?? $identity['serial_number'] ?? $id_number;
$district    = $ident_data['district']   ?? $identity['district']   ?? null;
$sub_county  = $ident_data['sub_county'] ?? $identity['sub_county'] ?? null;
$place_ob    = $ident_data['place_of_birth'] ?? $identity['place_of_birth'] ?? null;
$id_valid    = $ident_data['is_valid'] ?? $identity['is_valid'] ?? true;
...
$outstanding_bal  = (float)($delinquency['outstanding_balance'] ?? $report['total_outstanding_balance'] ?? 0);
$no_facilities    = (int)($delinquency['no_of_facilities'] ?? $report['total_accounts'] ?? 0);
$overdue_amount   = (float)($report['total_overdue_amount'] ?? $delinquency['overdue_amount'] ?? 0);
$npa_accounts     = (int)($report['npa_accounts'] ?? $report['total_npa'] ?? 0);
$performing       = (int)($report['performing_accounts'] ?? max(0, $no_facilities - $npa_accounts));
$accounts         = $report['account_info'] ?? $report['credit_accounts'] ?? $report['accounts'] ?? [];
$is_defaulting    = ($delinquency_code === '004' || $delinquency_code === '005');

$delinq_labels = [
    '001' => ['CRB Clear — No adverse history',           '#16a34a', 'fa-circle-check',        false],
    '002' => ['Good Standing — All loans repaid on time', '#0ea5e9', 'fa-thumbs-up',            false],
    '003' => ['Caution — Some late payments on record',   '#d97706', 'fa-clock',                false],
    '004' => ['Delinquent — Unpaid/defaulted loan(s)',    '#dc2626', 'fa-triangle-exclamation', true],
    '005' => ['Written Off — Debt written off by lender', '#991b1b', 'fa-ban',                  true],
];
[$delinq_label, $delinq_color, $delinq_icon, $delinq_bad] = $delinq_labels[$delinquency_code] ?? ['Status Unknown','#6b7280','fa-question',false];
...
.score-donut-wrap{position:relative;width:150px;height:150px;margin:0 auto;}
...
@media(max-width:480px){.result-hero{padding:28px 0 24px;}.result-hero h1{font-size:1.4rem;}.comp-grid{grid-template-columns:repeat(2,1fr) !important;}}
</style>
@endpush
...
<div class="rcard">
    ...
    <div class="rcard-body" style="text-align:center;">
        <div class="score-donut-wrap" style="width:200px;height:200px;">
...
<div class="rcard">
    <div class="rcard-head">
        <div class="rcard-icon" style="background:rgba({{ $delinq_bad ? '239,68,68' : '15,169,88' }},.1);color:{{ $delinq_color }};"><i class="fa-solid {{ $delinq_icon }}"></i></div>
        <h2>CRB Delinquency Status</h2>
        <span class="chip" style="background:{{ $delinq_color }}18;color:{{ $delinq_color }};border:1px solid {{ $delinq_color }}44;">Code {{ $delinquency_code }}</span>
    </div>
    <div class="rcard-body">
        <div style="background:{{ $delinq_color }}10;border:1px solid {{ $delinq_color }}30;border-radius:12px;padding:16px 18px;display:flex;align-items:center;gap:14px;margin-bottom:16px;">
            <div style="width:40px;height:40px;border-radius:50%;background:{{ $delinq_color }}20;color:{{ $delinq_color }};display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">
                <i class="fa-solid {{ $delinq_icon }}"></i>
            </div>
            <div>
                <div style="font-size:.95rem;font-weight:800;color:{{ $delinq_color }};margin-bottom:2px;">{{ $delinq_label }}</div>
                <div style="font-size:.8rem;color:var(--text-regular);line-height:1.5;">
                    @php
                    $delinq_expl = ['001'=>'No lender has ever reported a problem with your repayments. Your CRB record is completely clean.','002'=>'All your credit facilities are being serviced on time. You have an excellent repayment track record.','003'=>'Some late or missed payments appear on your record. You are not yet blacklisted but this is hurting your score.','004'=>'One or more of your loans are unpaid and a lender has flagged you on the CRB. This is blocking new loan approvals.','005'=>'A lender has written off your debt as uncollectable. This is the most severe CRB listing.'];
                    @endphp
                    {{ $delinq_expl[$delinquency_code] ?? 'Your CRB delinquency status has been retrieved.' }}
                </div>
            </div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;">
            <div style="text-align:center;background:#f8fafc;border:1px solid var(--border-color);border-radius:12px;padding:14px;">
                <div style="font-size:1.3rem;font-weight:800;color:var(--primary-navy);">{{ $no_facilities ?: '0' }}</div>
                <div style="font-size:.68rem;color:var(--text-light);margin-top:3px;">Credit Facilities</div>
            </div>
            <div style="text-align:center;background:#f8fafc;border:1px solid var(--border-color);border-radius:12px;padding:14px;">
                <div style="font-size:{{ $outstanding_bal > 0 ? '1rem' : '1.3rem' }};font-weight:800;color:{{ $outstanding_bal > 0 ? '#d97706' : '#16a34a' }};">
                    {{ $outstanding_bal > 0 ? 'KES ' . number_format($outstanding_bal) : 'None' }}
                </div>
                <div style="font-size:.68rem;color:var(--text-light);margin-top:3px;">Outstanding Balance</div>
            </div>
            <div style="text-align:center;background:#f8fafc;border:1px solid var(--border-color);border-radius:12px;padding:14px;">
                <div style="font-size:1rem;font-weight:800;color:{{ $delinq_color }};">{{ $delinquency_code }}</div>
                <div style="font-size:.68rem;color:var(--text-light);margin-top:3px;">Delinquency Code</div>
            </div>
        </div>
        @if($npa_accounts > 0 || $overdue_amount > 0)
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            ...
            <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:12px;text-align:center;">
                ...
                <div style="font-size:.7rem;color:#991b1b;margin-top:2px;">Non-Performing Accounts</div>
            </div>
            @endif
            @if($overdue_amount > 0)
            <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:12px;text-align:center;">
                <div style="font-size:1rem;font-weight:800;color:#c2410c;">KES {{ number_format($overdue_amount) }}</div>
                <div style="font-size:.7rem;color:#9a3412;margin-top:2px;">Total Overdue Amount</div>
            </div>
            @endif
        </div>
        @endif
...
<div class="rcard">
    ...
    <div class="rcard-body" style="padding:0;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.82rem;">
            <thead>
                <tr style="background:#f8fafc;border-bottom:2px solid var(--border-color);">
                    <th style="padding:10px 14px;text-align:left;font-size:.7rem;font-weight:700;text-transform:uppercase;color:var(--text-light);white-space:nowrap;">Lender / Institution</th>
                    <th style="padding:10px 14px;text-align:left;font-size:.7rem;font-weight:700;text-transform:uppercase;color:var(--text-light);">Type</th>
                    ...
                    <th style="padding:10px 14px;text-align:right;font-size:.7rem;font-weight:700;text-transform:uppercase;color:var(--text-light);">Overdue</th>
                    ...
                    <th style="padding:10px 14px;text-align:center;font-size:.7rem;font-weight:700;text-transform:uppercase;color:var(--text-light);white-space:nowrap;">Opened</th>
                </tr>
            </thead>
            <tbody>
            ...
            $is_npa      = stripos($acc_status,'npa')!==false || stripos($acc_status,'default')!==false || stripos($acc_status,'loss')!==false || $acc_overdue > 0;
            $status_color = $is_npa ? '#dc2626' : ($acc_overdue > 0 ? '#d97706' : '#16a34a');
            $status_bg    = $is_npa ? '#fef2f2' : ($acc_overdue > 0 ? '#fffbeb' : '#f0fdf4');
            $status_label = $acc_status ?: ($is_npa ? 'Non-Performing' : 'Performing');
            @endphp
            <tr style="border-bottom:1px solid var(--border-color);">
                <td style="padding:12px 14px;"><div style="font-weight:700;color:var(--primary-navy);font-size:.84rem;">{{ $lender }}</div></td>
                ...
                <td style="padding:12px 14px;text-align:right;font-weight:700;color:{{ $acc_overdue > 0 ? '#dc2626' : '#16a34a' }};">
        ...
        <div style="padding:14px 16px;background:#fffbeb;border-top:2px solid #fde68a;display:flex;justify-content:space-between;align-items:center;">
            <span style="font-size:.83rem;font-weight:700;color:#92400e;">Total Outstanding Balance</span>
            <span style="font-size:1.1rem;font-weight:800;color:#d97706;">KES {{ number_format($outstanding_bal) }}</span>
        </div>
        @endif
    </div>
</div>
...
<div class="rcard">
    <div class="rcard-head">
        ...
        @if($id_valid)<span class="chip chip-green"><i class="fa-solid fa-circle-check"></i> Verified</span>@endif
    </div>
    <div class="rcard-body">
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;">
            @foreach([['Full Name',$full_name,'fa-user'],['National ID',$id_serial,'fa-id-badge'],['Date of Birth',$dob ? \Carbon\Carbon::parse($dob)->format('d M Y') : null,'fa-cake-candles'],['Gender',$gender ? ucfirst(strtolower($gender)) : null,'fa-venus-mars'],['Citizenship',$citizenship,'fa-flag'],['District',$district,'fa-map-pin'],['Sub-County',$sub_county,'fa-location-dot'],['Place of Birth',$place_ob,'fa-house']] as [$label,$val,$icon])
            @if($val)
...
<div class="rcard">
    ...
    <div class="rcard-body">
        @if($credit_score >= 700)
        <div class="alert alert-green"><i class="fa-solid fa-circle-check" style="color:#16a34a;"></i><div class="alert-text"><strong>Excellent credit standing!</strong> You are in the top tier of borrowers. Lenders will be eager to approve your loan applications.</div></div>
        @elseif($credit_score >= 500)
        <div class="alert alert-yellow"><i class="fa-solid fa-triangle-exclamation" style="color:#d97706;"></i><div class="alert-text"><strong>Fair credit — room to improve.</strong> You may get approved by some lenders, but likely with higher interest rates.</div></div>
        @else
        <div class="alert" style="background:#fef2f2;border:1px solid #fecaca;"><i class="fa-solid fa-circle-xmark" style="color:#dc2626;"></i><div class="alert-text"><strong>Urgent action needed.</strong> Your credit score is below the standard lending threshold. Most lenders will decline applications at this level.</div></div>
        @endif
        <div style="display:grid;gap:10px;margin-top:4px;">
            @if($credit_score >= 700)
            @foreach([['fa-check','rgba(15,169,88,.1)','#16a34a','Eligible for Best Rates','You qualify for the lowest interest rates most lenders offer. Leverage this to negotiate.'],['fa-arrow-up','rgba(99,102,241,.1)','#6366f1','Keep It Up','Pay all loans on time and keep credit utilization low to maintain or improve your score.'],['fa-building-columns','rgba(14,165,233,.1)','#0ea5e9','Consider More Credit','Having a diverse credit mix (bank loans, digital credit, SACCOs) can help maintain your high score.']] as $tip)
            @elseif($credit_score >= 500)
            @foreach([['fa-calendar-check','rgba(245,158,11,.1)','#d97706','Pay On Time, Every Time','Payment history is the biggest factor. Set reminders for every loan repayment.'],['fa-chart-line','rgba(99,102,241,.1)','#6366f1','Reduce Debt Load','Keep your credit usage below 30% of total available credit across all accounts.'],['fa-clock','rgba(15,169,88,.1)','#16a34a','Allow Time','6 months of clean repayment behaviour can push your score into the Good tier.']] as $tip)
            @else
            @foreach([['fa-money-bill-wave','rgba(239,68,68,.1)','#dc2626','Clear Outstanding Debts First','Paying off defaulted loans is the single most effective way to improve your score quickly.'],['fa-handshake','rgba(99,102,241,.1)','#6366f1','Negotiate with Lenders','Contact your lenders to arrange repayment plans. Even partial settlement can help lift your CRB status.'],['fa-clock-rotate-left','rgba(15,169,88,.1)','#16a34a','Be Patient & Consistent','Consistent on-time payments over 12 months can significantly rebuild your credit profile.']] as $tip)
            @endif
            <div style="display:flex;gap:12px;align-items:flex-start;padding:12px;background:#f8fafc;border:1px solid var(--border-color);border-radius:10px;">
                <div style="width:32px;height:32px;border-radius:8px;background:{{ $tip[1] }};color:{{ $tip[2] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:.85rem;"><i class="fa-solid {{ $tip[0] }}"></i></div>
                <div><div style="font-weight:700;color:var(--primary-navy);font-size:.88rem;margin-bottom:2px;">{{ $tip[3] }}</div><div style="font-size:.78rem;color:var(--text-regular);line-height:1.5;">{{ $tip[4] }}</div></div>
            </div>
            @endforeach
        </div>
    </div>
...
<div class="sidebar">
    <div class="sidebar-card">
        ...
        <div class="sidebar-card-body">
            ...
            @if($delinquency_code)<div class="stat-row"><span class="stat-label">CRB Status</span><span class="stat-val" style="color:{{ $delinq_color }};font-size:.78rem;">{{ $delinq_bad ? 'Listed' : 'Clear' }}</span></div>@endif
            @if($no_facilities)<div class="stat-row"><span class="stat-label">Facilities</span><span class="stat-val">{{ $no_facilities }}</span></div>@endif
    ...
    <div class="sidebar-card no-print">
        ...
        <div class="sidebar-card-body">
            <button onclick="printPdfReport()" class="action-btn no-print"><i class="fa-solid fa-print" style="color:#6366f1;"></i> Print Report</button>
            <a href="{{ route('credit-score-check.pdf', $req->id) }}" target="_blank" class="action-btn" style="color:inherit;border:1px solid #bbf7d0;background:#f0fdf4;">
                <i class="fa-solid fa-file-pdf" style="color:#16a34a;"></i> <strong style="color:#15803d;">Download PDF Report</strong>
            </a>
            @php $pdfUrl = route('credit-score-check.pdf', $req->id); $waText = "My CRB Credit Score: {$credit_score}/900 ({$tier_label}).\n\nDownload my official report:\n{$pdfUrl}"; @endphp
            <a href="https://wa.me/?text={{ urlencode($waText) }}" target="_blank" class="action-btn" style="color:inherit;"><i class="fa-brands fa-whatsapp" style="color:#16a34a;"></i> Send PDF via WhatsApp</a>
...
function printPdfReport() {
    var win = window.open('{{ route('credit-score-check.pdf', $req->id) }}', '_blank');
