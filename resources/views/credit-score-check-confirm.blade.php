@extends('layouts.app')
@section('title', 'Confirm Credit Score Check - Readiwork')
        ...
        <div class="confirm-layout">
            <div class="info-card-l">
                <div style="display:flex;align-items:center;gap:14px;margin-bottom:24px;">
                    <div style="width:56px;height:56px;border-radius:14px;background:linear-gradient(135deg,#6366f1,#4f46e5);display:flex;align-items:center;justify-content:center;font-size:1.5rem;color:#fff;flex-shrink:0;">
                        <i class="fa-solid fa-gauge-high"></i>
                    </div>
                    <div>
                        <h2 style="font-size:1.2rem;font-weight:800;color:var(--primary-navy);margin-bottom:2px;">Credit Score Check</h2>
                ...
                <div style="display:grid;gap:10px;">
                    ...
                    <div style="display:flex;align-items:flex-start;gap:12px;padding:12px 14px;background:#f8fafc;border:1px solid var(--border-color);border-radius:10px;">
                        ...
                        <div>
                            <div style="font-size:.88rem;font-weight:600;color:var(--primary-navy);margin-bottom:2px;">{{ $f[2] }}</div>
                            <div style="font-size:.76rem;color:var(--text-light);">{{ $f[3] }}</div>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="order-card">
                ...
                <div class="order-card-body">
                    <div class="order-row"><span class="order-label">Service</span><span class="order-val">Credit Score Check</span></div>
                    <div class="order-row">
                        <span class="order-label">Credit Score</span>
                    ...
                    <p style="text-align:center;margin-top:14px;font-size:.85rem;"><a href="{{ route('credit-score-check') }}" style="color:var(--primary-green);font-weight:600;"><i class="fa-solid fa-arrow-left"></i> Run a different check</a></p>
                </div>
            </div>
        </div>
    </div>
...
function initiateStk() {
    ...
    fetch('{{ route('stk-push') }}', {
        ...
        body: JSON.stringify({ phone: raw, amount: {{ (int)$price }}, service: 'credit-score-check', id: ID_NUMBER, record_id: RECORD_ID }),
    })
    .then(r => r.json())
